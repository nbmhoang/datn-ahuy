...
let handleMessage = async (sender_psid, received_message) => {
    ...
    if (received_message.sticker_id) {
        await callSendAPI(sender_psid, "Cảm ơn bạn đã sử dụng dịch vụ của Bệnh viện Quốc tế Huế !!!");
        return;
    }
    ...
    switch (name) {
        case "intent":
            if (entity.value === 'doctors') {
                ...
                let title = "Bệnh viện Quốc tế Huế";
                let subtitle = 'Thông tin bác sĩ làm việc tại Bệnh viện Quốc tế Huế';
        ...
        case"booking":
            ...
            await callSendAPIv2(sender_psid, "Đặt lịch khám bệnh", "Hướng dẫn đặt lịch khám bệnh tại Bệnh viện Quốc tế Huế", BOOKING_IMAGE_URL, BOOKING_URL);
            await sendMessageDefault(sender_psid, "Xem thêm thông tin:");
            break;
        case"info":
            await callSendAPI(sender_psid, "Bạn đang tìm hiểu về thông tin website, xem thêm ở link bên dưới nhé.");
            await callSendAPIv2(sender_psid, "Thông tin website", "Thông tin website Bệnh viện Quốc tế Huế", INFOWEBSITE_IMAGE_URL, INFOWEBSITE_URL);
            await sendMessageDefault(sender_psid, "Xem thêm thông tin:");
            break;
        default:
            ...
            await callSendAPIv2(sender_psid, "Hỗ trợ khách hàng", "Thông tin hỗ trợ khách hàng Bệnh viện Quốc tế Huế", DEFAULT_IMAGE_URL, DEFAULT_URL);
            await sendMessageDefault(sender_psid, "Xem thêm thông tin:");
    }
};
...
let sendMessageReplyDoctors = (sender_psid) => {
    return new Promise(async (resolve, reject) => {
        try {
            let response1 = {
                "text": "Bệnh viện Quốc tế Huế tự hào mang đến cho bạn đội ngũ bác sĩ hàng đầu, chất lượng và uy tín." +
                    "\n\nMột số bác sĩ tiêu biểu của bệnh viện:"
            };
...
let sendMessageReplySpecialization = (sender_psid) => {
    return new Promise(async (resolve, reject) => {
        try {
            let response1 = {
                "text": "Bệnh viện Quốc tế Huế có đầy đủ các chuyên khoa khám chữa bệnh cho mọi lứa tuổi." +
                    "\n\nMột số chuyên khoa tiêu biểu của bệnh viện:"
            };
...
let sendMessageReplyClinics = (sender_psid) => {
    return new Promise(async (resolve, reject) => {
        try {
            let response1 = {
                "text": "Bênh viện quốc tế Huế có trên 20 bác sĩ hợp tác phát triển." +
                    "\n\nMột số bác sĩ hợp tác tiêu biểu với Bênh viện quốc tế Huế:"
            };
